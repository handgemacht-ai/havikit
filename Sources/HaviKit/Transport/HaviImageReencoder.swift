...
/// A retained handle to the frozen source image that can re-render it to encoded
/// bytes at a chosen format + longest side. It rides on `PendingAnnotation` so
/// the uploader can execute the two server-driven re-encode fallbacks (design
/// §4) — `unsupported_media_type` → PNG, `payload_too_large` → 1024 px — without
/// the actor ever touching UIKit. SDK-4 builds one over the freeze snapshot
