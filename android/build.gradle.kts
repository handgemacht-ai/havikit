...
//
// The Android Gradle Plugin (`android.library`) must be pinned here too. Kotlin's
// android plugin instantiates `KotlinAndroidTarget`, which references AGP's
// `com.android.build.gradle.api.BaseVariant`. With `kotlin.android` on the shared root
// classloader but AGP loaded only into :havikit's child classloader, that class is
// invisible and applying `kotlin.android` fails ("Could not create an instance of type
// KotlinAndroidTarget > com/android/build/gradle/api/BaseVariant"). Pinning AGP here
// hoists it onto the same classloader.
//
